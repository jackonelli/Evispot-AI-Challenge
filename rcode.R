...
subTrain = droplevels(subTrain)
#magic_vec = as.numeric(subTrain$TRANS_AMO%%1==0)
#magic_vec = as.numeric(subTrain$TRANS_AMO%%1==0)+1
magic_vec = as.factor(as.numeric(subTrain$TRANS_AMO%%1==0)+1)
subTrain$DEC = magic_vec
subTrain = subTrain[c(1:10,12,11)]
...
ccrf<-train(x=x.train,y=y.train,method="ranger",trControl = fitControl)
...
obs_user[i] <-which(usr_idx[,3] == train$Key_ENGNO[i])
}
key_dist[obs_user, ]
